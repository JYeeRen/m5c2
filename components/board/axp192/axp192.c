...
/**
  * @brief  获取 GPIO1 当前的工作模式
  * @retval 模式值 (0-7 对应寄存器 0x92 的低 3 位)
  */
uint8_t Axp192_GetGpio1Mode(void)
{
    // 读取 GPIO1 控制寄存器 (0x92)
    uint8_t reg_value = Axp192_Read8Bit(AXP192_GPIO1_CTL_REG);
    // 提取低 3 位 (模式位)
    return reg_value & 0x07;
}

/**
  * @brief  获取 GPIO1 当前的电平状态
  * @retval 0: 低电平, 1: 高电平
  */
uint8_t Axp192_GetGpio1Level(void)
{
    // 读取 GPIO0/1/2 状态寄存器 (0x94)
    uint8_t reg_value = Axp192_Read8Bit(AXP192_GPIO012_STATE_REG);
    // 提取 GPIO1 状态位 (bit1)
